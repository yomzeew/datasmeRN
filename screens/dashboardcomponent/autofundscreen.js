...
import { bankpreview } from "../services/endpoints"
...
const AutoFund = () => {
    ...
    const functiongetExp = async () => {
        ...
        if (checktoken === false) {
            ...
            try{
                setloader(true)
                const response=await axios.post(bankpreview, null, {
                    headers: {
                      Authorization: `Bearer ${mytokenreal}`,
                    }
                  });
                  console.log(response.data)
                  const account=response.data.accounts.bank
                  console.log(account)
                  setuserdata(account)
    ...
    return (
        <View className="h-full">
           ...
           <View className="absolute h-full w-full bg-slate-300 opacity-80"/>
